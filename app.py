...
senioridades_disponiveis = sorted(df['nivel_de_experiencia'].unique())
...
df_filtrado = df[
    ...
    (df['nivel_de_experiencia'].isin(senioridades_selecionadas)) &
...
with col_graf3:
    if not df_filtrado.empty:
        remoto_contagem = df_filtrado['taxa_remota'].value_counts().reset_index()
        remoto_contagem.columns = ['tipo_de_emprego', 'quantidade']
        grafico_remoto = px.pie(
            remoto_contagem,
            names='tipo_de_emprego',
            values='quantidade',
            color='tipo_de_emprego',
            template='plotly',
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        media_ds_pais = df_ds.groupby('residencia_do_empregado_iso3')['salario_em_usd'].mean().reset_index()
        grafico_paises = px.choropleth(media_ds_pais,
            locations='residencia_do_empregado_iso3',
            color='salario_em_usd',
            color_continuous_scale='rdylgn',
            title='Salário médio de Cientista de Dados por país',
            labels={'salario_em_usd': 'Salário médio (USD)', 'residencia_do_empregado_iso3': 'País'})
